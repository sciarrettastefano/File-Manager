...
Route::controller(\App\Http\Controllers\FileController::class)
    ...
    ->group(function () {
        Route::get('/my-files/{folder?}', 'myFiles')
            ->where('folder', '(.*)') // folder deve corrispondere alla regexp, che in questo caso specifica qualsiasi carattere (.), qualsiasi numero di volte (*)
            ->name('myFiles');
        Route::get('/trash', 'trash')->name('trash');
        Route::post('/folder/create', 'createFolder')->name('folder.create');
        Route::post('/file', 'storeHandlingVersions')->name('file.store');
